...
Ext.define('Ext.grid.plugin.BufferedRenderer', {
    ...
    bindStore: function (newStore) {
        ...
        if (view.componentLayout.layoutCount) {
            ...
            me.onViewResize(view, 0, view.getHeight());
        }
    },
    ...
    onRangeFetched: function(range, start, end, options, fromLockingPartner) {
        ...
        if (newRows && lockingPartner && !lockingPartner.disabled) {
            ...
            // Sync the row heights if configured to do so, or there are visible variableRowHeight columns
            if ((variableRowHeight || lockingPartner.variableRowHeight) && view.ownerGrid.syncRowHeight) {
    ...
    syncRowHeightsFinish: function () {
        ...
        // will trigger the needed refreshSize call.
    ...
    destroy: function() {
        ...
        Ext.destroy(me.viewListeners, me.storeListeners, me.stretcher);
        me.viewListeners = me.storeListeners = me.stretcher = null;
        me.callParent();
    }
}, function(cls) {
